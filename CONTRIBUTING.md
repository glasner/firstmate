...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - default to a local feature branch landed only after an explicit merge approval, and go through the `no-mistakes` pipeline and a PR only when a PR is explicitly requested for that change.
