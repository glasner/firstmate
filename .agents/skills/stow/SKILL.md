...
3. **Write within the existing boundaries.**
   ...
   - Knowledge general to every Firstmate user belongs in this repo's shared tracked material through the normal local-only branch and captain-approved landing path, or a no-mistakes PR only when the captain explicitly requests one.
